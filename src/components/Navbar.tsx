  ...
  return (
    <nav
      className={` snap-start top-0 xl:h-[10vh] left-0 w-full items-center justify-center z-50 transition-all duration-300 xl:py-0 py-3 shadow-[0_0_10px_0] border-b-[1px] border-custom-dark   
        ${openNavigation || scrolled ? "bg-custom-bg" : "bg-custom-bg"}`}
    >
          ...
          <div className="relative z-2 flex flex-col items-center justify-center m-auto lg:flex-row">
            {navigation.map((item) => (
              <Link
                ...
                to={item.url}
                onClick={handleClick}
                className={`group block relative font-light z-10 text-2xl uppercase text-custom-dark transition-colors  ${
                  item.onlyMobile ? "lg:hidden" : ""
                } px-6 py-4 md:py-8 xl:py-6 lg:-mr-0.25 lg:text-[0.9rem] lg:font-extrabold ${
